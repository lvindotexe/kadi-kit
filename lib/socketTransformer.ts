...
import { weaponTierOneMasterwork } from '../src/lib/types/weaponTypes.js';
import type {
	Catalyst,
	MasterWork,
	Mod,
...
import { getInvestmentStats } from './transformWeaponLite.js';
...
export function getCatalyst(
	...
	const reusablePlugItemHashes = entry.reusablePlugItems.map((plugItem) => plugItem.plugItemHash);

	const catalyst = catalysts.filter((cat) => reusablePlugItemHashes.includes(cat.hash))[0];
	const masterwork: DestinyInventoryItemDefinition | undefined = masterworks.filter((e) =>
		reusablePlugItemHashes.includes(e.hash)
	)[0];

	if (!catalyst || !masterwork) return null;
	const placeholder = catalysts.find((e) => e.hash == entry.singleInitialItemHash);
		...
		items: [
			{
				...
				hash: catalyst.hash,
				investmentStats: [...getInvestmentStats(catalyst, statDefs).values()],
...
export function getMasterWork(
	...
	const entry = filteredSocketEntries(item.sockets, 2685412949).filter((entry) =>
		weaponTierOneMasterwork.includes(entry.singleInitialItemHash)
	)[0];
	...
	masterworks =
		masterworkHashes.length == 0
			...
			: masterworks.filter(
					(mw) => masterworkHashes.includes(mw.hash) || mw.hash == entry.singleInitialItemHash
			  );
		...
		.map((mw): MasterWork => {
			return {
				...
				investmentStats: isAdept(item.displayProperties.name)
					? [...getInvestmentStats(mw, statDefs).values()]
					: [...getInvestmentStats(mw, statDefs).values()].filter((e) => e.value === 10)
			};
		});
...
export function getMods(
	...
	//TODO remove regex
	if (item.itemTypeAndTierDisplayName.toLocaleLowerCase().match(/^exotic/)) return null;
	const entry = filteredSocketEntries(item.sockets, 2685412949).filter(
	...
	const filteredMods = item.displayProperties.name.toLocaleLowerCase().match(/\(adept|timelost\)$/)
		? allMods.flat().reverse()
		: allMods.flat().filter((mod) => !mod.displayProperties.name.toLowerCase().match(/adept/));

	const items = filteredMods
		.filter((mod) => mod.hash != placeholder?.hash)
		.map((mod): Mod => {
			const perk = sandboxPerks.filter((e) => e.hash === mod.perks[0]?.perkHash)[0];
			return {
				...
				investmentStats: [...getInvestmentStats(mod, statDefs).values()]
			};
		});
...
export function getOrnaments(
	...
	const items = getPlugItems(entry.reusablePlugSetHash, plugsets, ornaments).map(
		(ornament): Ornament => {
			return {
				kind: 'Ornament',
...
export function getPerkHashes(
	...
	return getPerkSets(item, plugSets, weaponMods).flatMap((set) => set.map((perk) => perk.hash));
}
				...
				set.reduce(
					(acc, e) =>
						acc.set(e.hash, {
							...
							hash: e.hash,
							icon: e.displayProperties.icon,
							investmentStats: [...getInvestmentStats(e, statDefs).values()],
							kind: 'Trait',
...
const filteredSocketEntries = (
	...
	if (!sockets) throw 'incompatible item: missing DestinyItemSocketBlockDefinition';
			...
			return plugSet.reusablePlugItems
				.map((item) => {
					const mod = hayStackDefinitionTable.filter(
						(mod: DestinyInventoryItemDefinition) => mod.hash == item.plugItemHash
					)[0];
