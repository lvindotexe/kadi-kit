...
import { getDestinyManifest, getDestinyManifestComponent } from 'bungie-api-ts/destiny2';
import {
	getIntrinsicTrait,
	getDetailedPerkSets,
	getCatalyst,
...
import type { WeaponLite, Weapon } from './transformWeaponLite.js';
import {
	getInvestmentStatsLite,
	getInvestmentStatsLiteWithDisplayInterpolation
...
type TransformedTableNames =
	...
	| 'Traits'
	| 'WeaponLite';
type CategorisedItems = Map<TransformedTableNames, DestinyInventoryItemDefinition[]>;
...
function transformWeapon(
	...
	return {
		...
		investmentStats: getInvestmentStatsLiteWithDisplayInterpolation(item, statDefs, statGroups),
		sockets: {
			catalyst: getCatalyst(item, catalysts, masterworks, statDefs),
			mod: getMods(item, weaponMods, plugSets, statDefs, sandboxPerks),
			masterwork: getMasterWork(item, masterworks, statDefs),
...
function transformWeaponLite(
	...
	return {
		investmentStats: getInvestmentStatsLite(item, statGroups),
		hash: item.hash,
		name: item.displayProperties.name,
		icon: item.displayProperties.icon,
		...
		stats: Object.fromEntries(item.investmentStats.map((e) => [e.statTypeHash, e.value] as const)),
...
const httpClient = generateHttpClient(fetch, 'd1f18c12540e4df3b1cea9ee418fb234');
...
export function loadManifest() {
	return getManifestMetaData()
		.then(async (result) => {
			const manifestTables = await fetchManifestTables(
				[
					'DestinyInventoryItemDefinition',
			...
			//@ts-expect-error slutty stuff :)
			return manifestTables.set('version', result.version);
		})
		.then((results) => {
			...
			const categorisedItems = new Map<TransformedTableNames, DestinyInventoryItemDefinition[]>();
			...
			for (const [_, item] of Object.entries(itemDefs)) {
				...
				//sets weapons
				if (!item.itemCategoryHashes.includes(3109687656) && item.itemCategoryHashes.includes(1)) {
				...
				if (
					...
					item.itemTypeAndTierDisplayName.toLocaleLowerCase().match(/intrinsic$/) ||
				...
				if (item.plug?.plugCategoryIdentifier.toLocaleLowerCase().includes('masterwork')) {
		...
		.then(([manifestTables, categorisedItems]) => {
			...
			const [statGroups, statDefs, plugSets, sandboxPerks] = [
				manifestTables.get('DestinyStatGroupDefinition')! as DestinyStatGroupDefinition[],
				manifestTables.get('DestinyStatDefinition')! as DestinyStatDefinition[],
				manifestTables.get('DestinyPlugSetDefinition')! as DestinyPlugSetDefinition[],
				manifestTables.get('DestinySandboxPerkDefinition') as DestinySandboxPerkDefinition[]
			];
			const liteWeapons = new Array<WeaponLite>();
			const transformedWeapons = new Array<Weapon>();
			...
			for (const weapon of weapons) {
				...
				const result = transformWeaponLite(weapon, categorisedItems, plugSets, statGroups);
