import type { AllDestinyManifestComponents, DestinyManifest } from 'bungie-api-ts/destiny2';
import type { HttpClientConfig } from 'bungie-api-ts/http';
import { get, set } from 'idb-keyval';
import type { DatabaseTables } from '../src/lib/types/weaponTypes';
...
export function isAdept(name: string) {
	return ['harrowed', 'adept', 'timelost'].some((e) => name.toLowerCase().includes(e));
}
...
function getManifestTable<T extends keyof AllDestinyManifestComponents>(tableName: T) {
	const manifest: DestinyManifest = JSON.parse(localStorage.getItem('manifest')!);

	const tableUrl = manifest.jsonWorldComponentContentPaths.en![tableName]!;

	return fetch(`https://www.bungie.net${tableUrl}`)
		.then((result) => result.json())
		.then((result) => Object.values(result) as Array<AllDestinyManifestComponents[T][number]>);
}
	...
	return tables
		.map(async (tableName) =>
			tableName.includes('Destiny')
				? ([
						tableName,
						await getManifestTable(tableName as keyof AllDestinyManifestComponents)
				  ] as const)
				: fetch(`/api/${tableName}`)
						.then((response) => response.json())
						.then((response) => {
							return [tableName, Object.values(response) as DatabaseTables[T]] as const;
						})
		)
...
export async function fetchCached<T>(urlString: string, cacheKey: string) {
	const result = await get(cacheKey);
	if (!result) return fetchAndCache(urlString, cacheKey);
	return result;
}
